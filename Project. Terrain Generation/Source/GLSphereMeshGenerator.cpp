...
GLMesh GLSphereMeshGenerator::generate(const QColor& color)
{
	GLMesh mesh{};
...
void GLSphereMeshGenerator::init_vertices(std::vector<GLVertex>& vertices, const QColor& color)
{
	const auto PI = std::atanf(1.0) * 4.f;
    ...
    for (auto i = 0u; i <= stack_count_; ++i)
    {
        ...
        const auto xy = radius_ * std::cosf(stack_angle);
        const auto z = radius_ * std::sinf(stack_angle);
...
void GLSphereMeshGenerator::init_indices(std::vector<unsigned>& indices)
{
    for (auto i = 0u; i < stack_count_; ++i)
    {
