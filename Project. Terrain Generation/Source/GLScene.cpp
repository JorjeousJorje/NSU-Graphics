...
obj_sptr GLScene::create_object(ShaderData& data, GLMesh& mesh, const QVector3D& pos, GLMaterial& material) const
{
	auto object = std::make_shared<GLObject>();
	object->mesh = mesh;
	object->transform.translate(pos);
...
void GLScene::add_object(ShaderData& data, GLMesh& mesh, const QVector3D& pos, GLMaterial& material)
{
	objects.push_back(create_object(data, mesh, pos, material));
}
