...
inline QImage loadTga(const char* filePath)
{
	QImage img;
...
public:
	...
	static void load_texture(
	    ...
	    const QOpenGLTexture::Filter min = QOpenGLTexture::LinearMipMapLinear,
	    const QOpenGLTexture::Filter mag = QOpenGLTexture::Linear,
	    bool anisotropy = true, 
	    const std::string& normal_path = {})
	{
		GLTexture texture;
		if (texture_path.find(".tga") == std::string::npos) {
			auto image = QImage(texture_path.c_str());
			texture.texture_data = std::make_shared<QOpenGLTexture>(image);
		} else {
			auto image = loadTga(texture_path.c_str());
			texture.texture_data = std::make_shared<QOpenGLTexture>(image);
		}
		texture.texture_data->setMinMagFilters(min, mag);

		if(anisotropy) {
			texture.texture_data->setMaximumAnisotropy(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT);
		}
		...
		if (!normal_path.empty()) {
			GLTexture normal;
			normal.texture_data = std::make_shared<QOpenGLTexture>(QImage(normal_path.c_str()));
			texture.texture_data->setMinMagFilters(min, mag);
